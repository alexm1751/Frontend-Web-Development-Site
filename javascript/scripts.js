...
(function() {
  ...
  function toggleMenu(){
    if (navMenu.classList.contains('show-menu')) { // true: it's visible
      ...
      colour.classList.remove("fa-active"); // remove white hamburger css
      ...
      colour.classList.add("fa-active"); // add white hamburger css
...
(function() {
  ...
  function init() {
    ...
    about.addEventListener('click', (e) => { scrollTo(anchor1, e) }, false); /*Example: Here anchorpoint1 in the HTML has been 
  ...
  function scrollTo(to, callback, duration = 1500) { 
    ...
    let start = position(),
      change = to - start, /*The offsettop minus the start to calculate the distance of change*/
      currentTime = 0,
      increment = 20; /*This time changes the speed of the animation once it is entered into the bellow equation*/
